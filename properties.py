...
def get_properties():
    ...
    # Wholesalers only see properties matching their buy boxes
    if user.role != 'admin':
        user_boxes = BuyBox.query.filter_by(user_id=user_id, active=True).all()
        if user_boxes:
            matched_ids = db.session.query(PropertyMatch.property_id).filter(
    ...
    result = query.order_by(Property.created_at.desc()).paginate(page=page, per_page=per_page, error_out=False)
    return jsonify({
...
def get_stats():
    ...
    if user.role == 'admin':
        total = Property.query.filter_by(active=True).count()
    ...
    saved = PropertyMatch.query.join(BuyBox).filter(
